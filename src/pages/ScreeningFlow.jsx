...
export default function ScreeningFlow() {
  ...
  const isStepAccessible = (stepNum) => {
    switch (stepNum) {
      case 0: return true;
      case 1: return completedSteps.has(0);
      case 2: return completedSteps.has(1) && state.definition !== 'no';
      case 3: return completedSteps.has(2) && summary !== null;
  ...
  const summarySelectedTypes = summary?.selected_types || summary?.selectedTypes || [];
  const summaryRecommendations = summary?.recommended || summary?.recommended_tests || [];
  ...
  const sectionStyle = (stepNum) => ({
    ...
    border: '3px solid',
    borderColor: completedSteps.has(stepNum) ? '#10b981' : isStepAccessible(stepNum) ? '#d1d5db' : '#f3f4f6',
            ...
            <div key={step.num} style={{ display: 'flex', alignItems: 'center' }}>
              <button
                ...
                style={{
                  ...
                  alignItems: 'center',
                  padding: '0.5rem 1rem',
                  background: isCompleted ? '#10b981' : isAccessible ? '#3730a3' : '#e5e7eb',
              ...
              >
                ...
                <span style={{ fontSize: '0.75rem', fontWeight: 600, marginTop: '2px' }}>{step.label}</span>
              </button>
              {index < STEP_LABELS.length - 1 && (
                <div style={{
                  width: '24px',
                  height: '3px',
                  background: completedSteps.has(step.num) ? '#10b981' : '#e5e7eb',
                  margin: '0 4px',
                  borderRadius: '2px',
                }} />
              )}
  ...
  return (
    <div style={containerStyle}>
      ...
      {error && (
        <div className="alert alert-error" role="alert" style={{ marginBottom: '2rem', fontSize: '1.25rem', padding: '1.25rem' }}>
          {error}
        </div>
      )}
      ...
      <div ref={sectionRefs[0]} style={sectionStyle(0)}>
        ...
        <p style={textStyle}>
          In this screening, we'll check basic eligibility and learn about your synesthetic experiences.
          This should take about 3-5 minutes.
        </p>

        <label className="checkbox-group" htmlFor="consent" style={{ fontSize: '1.375rem', display: 'flex', alignItems: 'flex-start', gap: '1.25rem', marginBottom: '1.5rem' }}>
          <input
            id="consent"
            type="checkbox"
          ...
          />
          <span style={{ fontSize: '1.375rem', lineHeight: 1.5 }}>I consent to take part in this study.</span>
        </label>
        ...
        <p style={mutedTextStyle}>
          ...
          and use of your data for research purposes.
        </p>
      ...
      <div ref={sectionRefs[1]} style={sectionStyle(1)}>
        ...
        <p style={compactTextStyle}>
          Synesthesia is when one sense triggers another — like letters having colors or sounds having shapes.
        </p>

        <div className="grid-2" style={{ marginBottom: '0.75rem', fontSize: '1rem', gap: '1rem' }}>
          <div>
            <div className="label-uppercase" style={{ fontSize: '0.75rem', marginBottom: '0.25rem' }}>EXAMPLES</div>
          ...
          <div>
            <div className="label-uppercase" style={{ fontSize: '0.75rem', marginBottom: '0.25rem' }}>NOT SYNESTHESIA</div>
        ...
        <p style={{ ...compactTextStyle, fontWeight: 600, marginBottom: '0.5rem' }}>Do you experience synesthesia?</p>

        <div>
          <div className="choice-grid" style={{ marginBottom: '0.5rem', gap: '0.5rem' }}>
            <ChoiceCard
              title="Yes"
      ...
      <div ref={sectionRefs[2]} style={sectionStyle(2)}>
        ...
        <ul className="type-rows compact" style={{ fontSize: '1.125rem', margin: 0 }}>
          <TypeRow
            title="Letter • Color"
        ...
        <input
          ...
          style={{ fontSize: '1rem', padding: '0.625rem 1rem', marginTop: '0.5rem' }}
        />
      ...
      <div ref={sectionRefs[3]} style={sectionStyle(3)}>
        ...
        {!isStepAccessible(3) && (
          <p style={textStyle}>Complete the previous steps to see your results.</p>
        )}
        ...
        {isStepAccessible(3) && summary && (
          <>
            <p style={{ ...textStyle, fontSize: '1.625rem', fontWeight: 600, color: summary.eligible ? '#065f46' : '#374151' }}>
              {summary.eligible
            ...
            {summarySelectedTypes.length > 0 ? (
              <div className="summary-note" style={{ marginBottom: '1.5rem', padding: '1.25rem 1.75rem' }}>
                <div className="note-title" style={{ fontSize: '1.375rem', marginBottom: '1rem' }}>Selected types</div>
            ...
            {summaryRecommendations.length > 0 && (
              <>
                <div className="summary-title mt-3" style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '1rem' }}>
                  Recommended Tests
                </div>
                <div className="summary-grid" style={{ gap: '1rem' }}>
                  {summaryRecommendations.map((rec, idx) => (
                    <div className="summary-card" key={`${rec.name}-${idx}`} style={{ padding: '1.5rem' }}>
                      <div className="summary-title" style={{ fontSize: '1.375rem' }}>{rec.name}</div>
                      {rec.reason && (
                        <div className="summary-sub" style={{ fontSize: '1.125rem' }}>{rec.reason}</div>
                      )}
                    </div>
                  ))}
                </div>
