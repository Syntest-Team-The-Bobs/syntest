...
export default function TestComplete({
	onNext,
	isDone = false,
	analysisResult: initialResult = null,
}) {
			...
			.then(async (res) => {
				...
				if (contentType.includes("application/json")) {
					...
					if (!res.ok) {
						...
						setError(
							`Unexpected non-JSON response from analysis endpoint: ${txt}`,
						);
					}
				}
			})
			...
			<div style={{ color: "#374151" }}>
				...
				>
					<p style={{ fontSize: "1.25rem", margin: 0 }}>
						<strong>
							{isSynesthete ? "✓ Synesthete" : "○ Non-Synesthete"}
						</strong>
					</p>
					<p style={{ marginTop: 6, color: "#6b7280", margin: "6px 0 0 0" }}>
						Consistency score:{" "}
						<strong>
							{participant.participant_score?.toFixed?.(3) ?? "—"}
						</strong>
					</p>
					<p style={{ marginTop: 6, color: "#6b7280", margin: "6px 0 0 0" }}>
		...
		>
			<div style={{ maxWidth: "900px", margin: "0 auto", textAlign: "center" }}>
				...
				{loading ? (
					...
					<div>
						<p style={{ fontSize: "1.125rem", color: "#b91c1c" }}>
							Could not run analysis: {error}
						</p>
						<p
							style={{ fontSize: "1rem", color: "#4b5563", marginTop: "1rem" }}
						>
