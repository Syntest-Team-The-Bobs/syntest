  ...
  return (
    <div style={{ minHeight: '100vh', padding: '3rem', background: '#f8fafc' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
        <h1 style={{ fontSize: '2rem', marginBottom: '1rem' }}>Analysis Results</h1>
        {p.status !== 'ok' ? (
          ...
          <div>
            ...
            <p style={{ color: '#374151' }}>Consistency score: {p.participant_score?.toFixed?.(3) ?? '—'}</p>
              ...
              {Object.keys(per).length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
                  {Object.entries(per).map(([k, v]) => (
                    <div key={k} style={{ background: 'white', padding: '0.75rem', borderRadius: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                          <div style={{ fontWeight: 700 }}>{k}</div>
                          <div style={{ fontSize: '0.9rem', color: '#6b7280' }}>{v.status}</div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          {v.representative_hex ? (
                            <div style={{ width: 36, height: 36, borderRadius: 6, background: v.representative_hex, border: '1px solid rgba(0,0,0,0.08)' }} />
                          ) : null}
                          <div style={{ fontSize: '0.85rem', color: '#374151' }}>{v.representative_hex ?? ''}</div>
                        </div>
                      </div>
            ...
            <div style={{ marginTop: '2rem' }}>
              <button
                onClick={onProceed}
                style={{ background: '#2563eb', color: 'white', padding: '0.75rem 1.75rem', borderRadius: 6, border: 'none', cursor: 'pointer' }}
              >
