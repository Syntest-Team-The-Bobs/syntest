...
class TestResult(db.Model):
    ...
    status = db.Column(
        db.String(50), default="not_started"
...
class ScreeningSession(db.Model):
    ...
    # lifecycle
    status = db.Column(
        db.String(20), default="in_progress", index=True
    )  # in_progress|completed|exited
    exit_code = db.Column(
        db.String(8), nullable=True, index=True
    )  # A | BC | D | NONE (or NULL if eligible)
    consent_given = db.Column(db.Boolean, default=False, nullable=False)

    # derived outcome
    eligible = db.Column(db.Boolean, nullable=True)
    selected_types = db.Column(
        db.JSON, nullable=True
    )  # e.g., ["Grapheme – Color", "Lexical – Taste"]
    recommended_tests = db.Column(
        db.JSON, nullable=True
    ...
    def record_event(self, step: int, event: str, details: dict | None = None):
        self.events.append(
            ScreeningEvent(step=step, event=event, details=details or {})
        )
        return self
...
class ScreeningHealth(db.Model):
    ...
    session_id = db.Column(
        db.Integer, db.ForeignKey("screening_sessions.id"), nullable=False, index=True
    )
...
class ScreeningRecommendedTest(db.Model):
    ...
    suggested_name = db.Column(
        db.String(128), nullable=False
    )  # human/lookup name (e.g., 'Grapheme-Color')
    reason = db.Column(db.String(255), nullable=True)

    test_id = db.Column(
        db.Integer, db.ForeignKey("tests.id"), nullable=True, index=True
    )
...
class ColorStimulus(db.Model):
    ...
    owner_researcher_id = db.Column(
        db.Integer, db.ForeignKey("researchers.id"), nullable=True, index=True
    )
    ...
    test_data_entries = db.relationship(
        "TestData", back_populates="stimulus", lazy="dynamic"
    )
    analyzed_data_entries = db.relationship(
        "AnalyzedTestData", back_populates="stimulus", lazy="dynamic"
    )
...
class ColorTrial(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            "selected_r >= 0 AND selected_r <= 255", name="check_selected_r_range"
        ),
        CheckConstraint(
            "selected_g >= 0 AND selected_g <= 255", name="check_selected_g_range"
        ),
        CheckConstraint(
            "selected_b >= 0 AND selected_b <= 255", name="check_selected_b_range"
        ),
    ...
    def selected_hex_color(self):
        ...
        if all(
            v is not None for v in [self.selected_r, self.selected_g, self.selected_b]
        ):
    ...
    def color_distance(self):
        ...
        if self.stimulus and all(
            v is not None for v in [self.selected_r, self.selected_g, self.selected_b]
        ):
            return self.stimulus.distance_to(
                self.selected_r, self.selected_g, self.selected_b
            )
        return None
    ...
    def is_exact_match(self):
        ...
        if self.stimulus and all(
            ...
            return (
                self.stimulus.r == self.selected_r
                and self.stimulus.g == self.selected_g
                and self.stimulus.b == self.selected_b
            )
        return False
...
class TestData(db.Model):
    ...
    stimulus = db.relationship(
        "ColorStimulus", back_populates="test_data_entries", lazy="joined"
    )
...
class ScreeningTestData(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(
        db.Integer, db.ForeignKey("participants.id"), index=True, nullable=False
    )
    participant = db.relationship(
        "Participant", backref="screening_test_data", lazy="joined"
    )
...
class SpeedCongruency(db.Model):
    ...
    )  # e.g., 'word', 'image', etc. (future-proof)
    ...
    )  # did chosen color match expected association?
...
class AnalyzedTestData(db.Model):
    ...
    participant = db.relationship(
        "Participant", backref="analyzed_test_data", lazy="joined"
    )
    ...
    stimulus = db.relationship(
        "ColorStimulus", back_populates="analyzed_data_entries", lazy="joined"
    )
