...
def get_dashboard_data():
    try:
        ...
        user = (
            Participant.query.get(user_id)
            if role == "participant"
            else Researcher.query.get(user_id)
        )
        ...
        if role == "participant":
            ...
            if latest_screening:
                ...
                recs = (
                    ScreeningRecommendedTest.query.filter_by(
                        session_id=latest_screening.id
                    )
        ...
        if role == "participant":
            ...
            tests_pending = len(
                [
                    ...
                    if tr.status in ["not_started", "in_progress"]
                ]
            )
                ...
                for rec in recommended_tests:
                    if rec.get("test_id"):
                        existing = TestResult.query.filter_by(
                            participant_id=user_id, test_id=rec.get("test_id")
                        ).first()
                        if not existing:
                            tests_pending += 1
        ...
        completion_percentage = (
            int((tests_completed / total_tests * 100)) if total_tests > 0 else 0
        )
