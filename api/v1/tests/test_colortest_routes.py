...
class TestSaveColorTrialsBatch:
    ...
    def test_batch_save_multiple_trials(self, client, auth_participant):
        ...
        trials = ColorTrial.query.filter(
            ColorTrial.id.in_(data['trial_ids'])
        ).all()
        assert len(trials) == 3
...
class TestColorTestIntegration:
    ...
    def test_complete_letter_test_workflow(self, client, auth_participant):
        ...
        session_response = client.post(
            '/api/v1/color-test/session/start',
            json={"test_type": "letter"}
        )
...
class TestErrorHandling:
    ...
    def test_start_session_exception_handling(self, client, auth_participant, monkeypatch):
