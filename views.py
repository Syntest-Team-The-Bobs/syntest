...
def _current_participant_id():
    # Replace with your auth; for dev, stash a demo PID in session
    pid = session.get("pid")
    if pid:
        return pid
    p = Participant(name="Demo User", email=f"demo{datetime.utcnow().timestamp()}@x.com",
                    password_hash="dev", age=None, country=None)
...
def save_step1():
    ...
    db.session.add(h); s.record_event(1, "save", payload); db.session.commit()
...
def save_step2():
    ...
    val = (payload := request.get_json(force=True)).get("answer", "yes")
    d.answer = YesNoMaybe(val)
    db.session.add(d); s.record_event(2, "save", payload); db.session.commit()
    return jsonify(ok=True)
...
def save_step3():
    ...
    pe.answer = YesNo(request.get_json(force=True).get("answer", "no"))
    db.session.add(pe); s.record_event(3, "save", {"answer": pe.answer.value}); db.session.commit()
    return jsonify(ok=True)
...
def save_step4():
    ...
    db.session.add(tc); s.record_event(4, "save", j); db.session.commit()
    return jsonify(ok=True)
...
def finalize_screening():
    ...
    s.finalize()
    db.session.commit()
    return jsonify(ok=True, eligible=s.eligible, exit_code=s.exit_code,
                   selected_types=s.selected_types, recommended=s.recommended_tests)
